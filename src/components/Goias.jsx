...
const Goias = () => {
  ...
  const images = [
    '/images/geral/goias.webp',
    '/images/geral/goias.1.webp',
  ];
  ...
  const styles = {
    ...
    h2: { fontFamily: `'Georgia', serif`, fontSize: 'clamp(2rem, 4vw, 2.8rem)', color: darkMode ? '#d4a84b' : '#8b5e1a', marginBottom: '1.5rem', fontWeight: 400 },
    pWithBorder: { fontSize: 'clamp(1rem, 2vw, 1.15rem)', lineHeight: 1.8, color: darkMode ? '#adb5bd' : '#495057', paddingLeft: '1.5rem', borderLeft: `3px solid ${darkMode ? '#c8860a' : '#8b5e1a'}` },
    ctaContainer: { textAlign: 'center', background: darkMode ? '#161b22' : '#f8f9fa', padding: '6rem 2rem' },
    ctaButton: { background: 'linear-gradient(45deg, #8b5e1a, #c8860a)', color: 'white', border: 'none', padding: '1.2rem 3.5rem', borderRadius: '50px', cursor: 'pointer', fontSize: '1.25rem', fontWeight: 'bold', transition: 'transform 0.3s ease, box-shadow 0.3s ease', boxShadow: '0 5px 20px rgba(139,94,26,0.4)', display: 'inline-block' },
  ...
  return (
    <div style={styles.page}>
      ...
      <main>
        <section style={styles.hero}>
          ...
          <div style={styles.heroContent}>
            <h1 style={styles.h1}>Goiás: Coração do Brasil</h1>
            <p style={styles.heroP}>Terra de cerrado, cachoeiras cristalinas, cidades históricas e uma gastronomia que aquece a alma. Goiás é o coração pulsante do Brasil Central.</p>
          </div>
        </section>
        ...
        <section style={styles.featureSection} className="feature-section-animate">
          <div style={styles.featureImageContainer} className="feature-image-container">
            <img src="/images/geral/goias.2.jpg" alt="Chapada dos Veadeiros" style={styles.featureImage} className="feature-image" />
          </div>
          <div style={styles.featureText}>
            <h2 style={styles.h2}>História Viva no Cerrado</h2>
            <p style={styles.pWithBorder}>Goiás Velho, Patrimônio Mundial da UNESCO, preserva ruas de pedra, igrejas barrocas e casarões coloniais que contam a história do ciclo do ouro no Brasil. A cidade é um dos conjuntos arquitetônicos coloniais mais bem preservados do país.</p>
          </div>
        </section>

        <section style={{ ...styles.featureSection, flexDirection: 'row-reverse' }} className="feature-section-animate">
          <div style={styles.featureImageContainer} className="feature-image-container">
            <img src="/images/geral/goias - natureza.jpg" alt="Natureza de Goiás" style={styles.featureImage} className="feature-image" />
          </div>
          <div style={styles.featureText}>
            <h2 style={styles.h2}>Natureza do Cerrado</h2>
            <p style={styles.pWithBorder}>A Chapada dos Veadeiros, Patrimônio Natural da Humanidade, encanta com cachoeiras monumentais, trilhas no cerrado e uma energia mística única. O estado abriga ainda o Parque Nacional das Emas e inúmeras cachoeiras de águas cristalinas.</p>
          </div>
        </section>

        <section style={styles.featureSection} className="feature-section-animate">
          <div style={styles.featureImageContainer} className="feature-image-container">
            <img src="/images/geral/goias - comida.jpg" alt="Gastronomia goiana" style={styles.featureImage} className="feature-image" />
          </div>
          <div style={styles.featureText}>
            <h2 style={styles.h2}>Sabores do Sertão</h2>
            <p style={styles.pWithBorder}>A culinária goiana é uma das mais ricas do Brasil Central. O empadão goiano, a pamonha, o pequi com arroz e a galinhada são pratos que expressam a identidade de um povo hospitaleiro, com raízes indígenas e portuguesas que se fundiram no coração do Brasil.</p>
          </div>
        </section>
        ...
        <section style={styles.ctaContainer}>
          <button
            ...
            onClick={() => navigate('/goias-pontos')}
            onMouseOver={(e) => { e.currentTarget.style.transform = 'scale(1.05)'; e.currentTarget.style.boxShadow = '0 8px 25px rgba(139,94,26,0.6)'; }}
            onMouseOut={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.boxShadow = '0 5px 20px rgba(139,94,26,0.4)'; }}
          >
            Explore Goiás
          </button>
        </section>
      ...
      <footer style={styles.footer}>
        <p>&copy; 2025 GADYS. Descubra Goiás.</p>
      </footer>
    </div>
  );
};
