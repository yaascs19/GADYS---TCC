      ...
      <header className="header" style={{ background: darkMode ? 'rgba(15,12,41,0.8)' : '#FFFFFF', backdropFilter: 'blur(30px)', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 100, borderBottom: darkMode ? '1px solid rgba(255,255,255,0.1)' : '1px solid #e0e0e0' }}>
        <nav className="nav" style={{ display: 'contents' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <img onClick={() => navigate('/')} style={{cursor:'pointer', height:'40px'}} src="/images/logos/logo.png" alt="GADYS" className="logo"  />
          ...
          <ul className="nav-links" style={{background: darkMode ? 'rgba(15,12,41,0.79)' : 'white', paddingTop: '5rem', justifyContent: 'flex-start', gap: '2rem'}}>
            ...
            <li className="dropdown">
              ...
              <div className="dropdown-content" style={{backgroundColor: darkMode ? 'rgb(252,252,252)' : '#f9f9f9', border: darkMode ? '1px solid rgba(255,255,255,0.1)' : '1px solid #ddd'}}>
                <Link to="/acre" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Acre</Link>
                <Link to="/amapa" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Amapá</Link>
                <Link to="/amazonas-estado" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Amazonas</Link>
                <Link to="/ceara" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Ceará</Link>
                <Link to="/para" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Pará</Link>
                <Link to="/rio-de-janeiro" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Rio de Janeiro</Link>
                <Link to="/rondonia" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Rondônia</Link>
                <Link to="/roraima" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Roraima</Link>
                <Link to="/sao-paulo" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>São Paulo</Link>
                <Link to="/tocantins" onClick={() => document.querySelector('.nav-links').classList.remove('active')} style={navLink(darkMode ? 'white' : 'black')}>Tocantins</Link>
              </div>
            </li>
